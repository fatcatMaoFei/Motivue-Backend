...
# Evidence Weights for Fitness Assessment - 证据加权机制
EVIDENCE_WEIGHTS_FITNESS = {
    "hrv_trend": 1.0,              # HRV - 最重要的客观指标
    "restorative_sleep": 0.95,     # 恢复性睡眠 - 核心客观指标
    "sleep_performance": 0.90,     # 睡眠表现 - 客观传感器数据
    "subjective_fatigue": 0.75,    # 主观疲劳 - 重要主观指标
    "subjective_stress": 0.70,     # 主观压力 - 心理因素
    "muscle_soreness": 0.65,       # 肌肉酸痛 - 主观感受
    "subjective_sleep": 0.60,      # 主观睡眠质量 - 补充指标
    "nutrition": 0.60,             # 营养状况 - 间接影响因子
    "gi_symptoms": 0.50,           # 肠胃症状 - 次要指标
    "fatigue_3day_state": 0.85     # 3天累积疲劳 - 重要趋势指标
}
# Interaction CPT for muscle_soreness and subjective_stress
INTERACTION_CPT_SORENESS_STRESS = {
    # muscle_soreness, subjective_stress 的所有组合
    # 关键组合：低酸痛+高压力 = 神经疲劳(NFOR)征象
...
class DBN_Engine:
    ...
    def run_bayesian_update(self, prior_probs, symptom_evidence, weights=None):
        posterior_probs = prior_probs.copy()
        if weights is None:
            weights = EVIDENCE_WEIGHTS_FITNESS

        # 首先处理肌肉酸痛和主观压力的交互证据
        soreness = symptom_evidence.get("muscle_soreness")
        stress = symptom_evidence.get("subjective_stress")
        
        # 方案A：优先交互，后备独立
        interaction_processed = False
        if soreness is not None and stress is not None:
            # 两个证据都存在：优先使用交互CPT（更精确）
            interaction_key = (soreness, stress)
        ...
        # 处理其他独立证据（包括单独的muscle_soreness或subjective_stress）
        for evidence_type, value in symptom_evidence.items():
            # 如果已经通过交互CPT处理了，就跳过独立处理
...
if __name__ == '__main__':
    ...
    previous_day_probabilities = {
        'Peak': 0, 'Well-adapted': 0.5, 'FOR': 0.3,
        'Acute Fatigue': 0.1, 'NFOR': 0.1, 'OTS': 0.0
    }
    ...
    symptom_inputs = {
        ...
        'hrv_trend': 'stable',  # 使用新的'stable'状态
